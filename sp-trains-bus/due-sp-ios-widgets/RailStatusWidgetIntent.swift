...
enum RailWidgetLineOption: String, CaseIterable, AppEnum {
    case metro1 = "metro-1"
    case metro2 = "metro-2"
    case metro3 = "metro-3"
    case metro4 = "metro-4"
    ...
    static var caseDisplayRepresentations: [RailWidgetLineOption: DisplayRepresentation] {
        [
...
struct RailStatusWidgetIntent: WidgetConfigurationIntent {
    static var title: LocalizedStringResource = "Follow Rail Lines"
    static var description = IntentDescription("Choose which rail lines to highlight in the status widget.")

    @Parameter(title: "Lines")
    var lines: [RailWidgetLineOption]?
}

extension RailStatusWidgetIntent {
    var selectedLineKeys: [String] {
        var keys: [String] = []
        for key in (lines ?? []).map(\.rawValue) where !keys.contains(key) {
            keys.append(key)
        }
        return keys
    }
